...
class SimulatedTournament(BaseModel):
    scores: list[Score]
    # Validates that all scores have the same users used in scoring
    # Validates that all forecasts have a score
    # Validates that all users have a forecast
    # Validates that all forecasts have both baseline and spot scores

    @property
    def users(self) -> set[User]:
        users = set()
        for score in self.scores:
            users.update(score.users_used_in_scoring)
        return users
    ...
    def questions(self) -> set[Question]:
        questions = set()
        for score in self.scores:
            questions.add(score.forecast.question)
        return questions
    ...
    def forecasts(self) -> list[Forecast]:
        forecasts = []
        for score in self.scores:
            forecasts.extend(score.forecast)
        return forecasts
...
class Score(BaseModel):
    ...
    forecast: Forecast
    users_used_in_scoring: list[User] # Empty if baseline
...
class Question(BaseModel):
    question_text: str
    resolution: Literal["yes", "no", "annulled", "ambiguous"]
    weight: float
